...
public class MyCollectionBuyPresenter extends BasePresenter<ICollectionBuyView> {

    private String mNextSearchStart;
    private int mRealTotalSize;
    private CollectionBuyListAdapter mLoadMoreAdapter;
    private Map<String, Object> map;

    public MyCollectionBuyPresenter(BaseActivity context) {
        super(context);
    }

    //买过水帖列表
    public void getBuyPostList(final boolean isRefresh,final boolean isLoadMore) {
        if (isLoadMore) {
            map.put(RequestParams.search_start, mNextSearchStart);
        } else {
            map = new HashMap<>();
        }
        Observable<BaseResponse<CollectionSTEntry>> observable = HttpUtil.getInstance().sApi.getBuyPostList(RequestBodyUtils.build(map));
    ...
    public void setLoadMoreAdapter(CollectionBuyListAdapter mAdapter) {
