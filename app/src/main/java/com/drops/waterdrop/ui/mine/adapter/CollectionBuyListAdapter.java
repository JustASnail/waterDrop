...
public class CollectionBuyListAdapter extends BaseQuickAdapter<CollectionSTEntry.ResultsBean, BaseViewHolder> {
    ...
    public CollectionBuyListAdapter(@LayoutRes int layoutResId) {
    ...
    private void setLayoutVisible(final BaseViewHolder helper, final CollectionSTEntry.ResultsBean item) {
        ...
        Log.d(TAG, "setLayoutVisible: " + collectStatus + isPrivateMode);
        ...
        ivDelete.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                deleteCollection(s, 1, helper.getPosition());
            }
        });
    ...
    private void setPrivate(String collectIdJson, int type) {
        ...
        Observable<BaseResponse<Object>> observable = HttpUtil.getInstance().sApi.buyPostHide(RequestBodyUtils.build(map));
    ...
    private void cancelPrivate(String collectIdJson, int type) {
        ...
        map.put(RequestParams.collect_id_json, collectIdJson);
        map.put(RequestParams.type, type);
        Observable<BaseResponse<Object>> observable = HttpUtil.getInstance().sApi.buyPostShow(RequestBodyUtils.build(map));
        HttpUtil.getInstance().execute(observable, new ProgressSubscriber<Object>(mContext) {
            @Override
            protected void _onNext(Object dataBean) {
    ...
    private void deleteCollection(String collectIdJson, int type, final int pos) {
        HashMap<String, Object> map = new HashMap<>();
        map.put(RequestParams.collect_id_json,collectIdJson);
        Observable<BaseResponse<Object>> observable = HttpUtil.getInstance().sApi.deleteBuyPost(RequestBodyUtils.build(map));
        HttpUtil.getInstance().execute(observable, new ProgressSubscriber<Object>(mContext) {
            @Override
            protected void _onNext(Object dataBean) {
    ...
    private CollectionListAdapter.OnDeleteListener onDeleteListener;

    public void setOnDeleteListener(CollectionListAdapter.OnDeleteListener onDeleteListener) {
