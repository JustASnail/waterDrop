...
import com.drops.waterdrop.ui.session.adapter.OrderMsgListAdapter;
...
public class OrderMessageListActivity extends BaseActivity {
    ...
    public static void start(Context context) {
        Intent starter = new Intent(context, OrderMessageListActivity.class);
    ...
    protected void initView() {
        ...
        options.titleString = "交易物流信息";
        setMyToolbar(options);

        mAdapter = new OrderMsgListAdapter(0);
    ...
    protected void initData() {
        ...
        List<SystemMessageDB> orderList = dbDao.queryBuilder().where(SystemMessageDBDao.Properties.Type.eq(21)).list();

        if (orderList != null) {
            sortRecentContacts(orderList);
...
//                        OrderDetailActivity.start(OrderMessageListActivity.this, systemMessageDB.getTargetId());
    ...
    public void onSystemMessageEvent(OrderMsgEvent event) {
        SystemMessageDBDao dbDao = GreenDaoManager.getInstance().getmDaoSession().getSystemMessageDBDao();
        List<SystemMessageDB> orderList = dbDao.queryBuilder().where(SystemMessageDBDao.Properties.Type.eq(21)).list();//订单消息
