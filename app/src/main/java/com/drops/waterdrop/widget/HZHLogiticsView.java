...
/**
 * Created by HZH on 2017/7/9.
 * 使用的时候如果在当前页面获取数据在展示，物流信息展示不出来的话
 * 可以在上个界面获取完成后，将数据带过来就可以了
 * Role:自定义的物流信息界面
 */
public class HZHLogiticsView extends View {
     ...
     * 绘制参数
     */
    int interval;//间隔：本文中指代物流信息和物流时间的距离
    float radius;//圆形半径
    int left = 60;//距离左边边距,圆点左边
    int top = 35;//距离顶部边距（相当于XML中的Margin或者Padding效果）
     ...
     * 绘制界面参数
     */
    List<LogiticsEntity.LogisticsBean> logisticsDataList;
    ...
    private float CONTENT_TEXT_SIZE = 60f;
    private float TIME_TEXT_SIZE = 30f;
    private float POINT_HEIGHT = 70f;   //圆点的相对高度，与第一行字对齐

    public HZHLogiticsView(Context context) {
        super(context);
        this.context = context;
        init();
    }

    public HZHLogiticsView(Context context, AttributeSet attrs) {
        super(context, attrs);
        this.context = context;
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.LogisticsInfoView);
        width = typedArray.getDimension(R.styleable.LogisticsInfoView_width_in, 5);//获取XML中设置的高度（圆形半径）
        radius = typedArray.getDimension(R.styleable.LogisticsInfoView_radius_in, 10);
    ...
    public HZHLogiticsView(Context context, AttributeSet attrs, int defStyleAttr) {
     ...
     */
    private void init() {
        CONTENT_TEXT_SIZE = dip2px(context,14);
        TIME_TEXT_SIZE = dip2px(context,12);
        POINT_HEIGHT = dip2px(context, 18);
        left = dip2px(context, 20);

        paint = new Paint(Paint.ANTI_ALIAS_FLAG);
        paint.setColor(getResources().getColor(R.color.color_gray_d9d9d9));

        mPaintPhone = new Paint(Paint.ANTI_ALIAS_FLAG);
        mPaintPhone.setColor(getResources().getColor(R.color.colorPrimary));
        mPaintPhone.setTextSize(CONTENT_TEXT_SIZE);

        textPaintPhone = new TextPaint();
        textPaintPhone.setColor(getResources().getColor(R.color.colorPrimary));
        textPaintPhone.setTextSize(CONTENT_TEXT_SIZE);
        textPaintPhone.setAntiAlias(true);

        interval = dip2px(context, 16);
     ...
     */
    public void setLogisticsDataList(List<LogiticsEntity.LogisticsBean> logisticsDataList) {
        ...
        for (int i = 0; i < logisticsDataList.size(); i++) {
            String content = (logisticsDataList.get(i)).getContent();
            StaticLayout layout = new StaticLayout(content + "", textPaint, (int) (windowWidth * 0.8), Layout.Alignment.ALIGN_NORMAL, 1.0F, 0.0F, true);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < logisticsDataList.size(); i++) {
            ...
            if (i == 0) {
                ...
                //画文字
                mPaint.setTextSize(TIME_TEXT_SIZE);
                canvas.drawText(((LogiticsEntity.LogisticsBean) data.get(i)).getTime() + "", left * 2 + radius*2, heightList.get(i) + interval, mPaint);
                //文字换行
                TextPaint textPaint = new TextPaint();
...
//                StaticLayout layout = new StaticLayout(((LogiticsEntity.LogisticsBean)data.get(i)).getContent()+"", textPaint, (int) (windowWidth * 0.8), Layout.Alignment.ALIGN_NORMAL, 1.0F, 0.0F, true);
                 ...
                 * 分割带电话的数据
                 * */
                String[] splitData = splitString(((LogiticsEntity.LogisticsBean) data.get(i)).getContent() + "");
                if (splitData != null) {
                    //说明有电话号码
                    ...
                    canvas.translate(left * 2 + radius * 2, 0);
                    layout.draw(canvas);
                ...
                paint.setColor(getResources().getColor(R.color.color_gray_d9d9d9));
                //圆点的位置
                canvas.drawCircle(width / 2 + left, heightData + POINT_HEIGHT, radius, paint);
                canvas.drawLine(left * 2 + radius * 2, heightData, windowWidth, heightData, paint); //画线
                //画文字
                paint.setTextSize(TIME_TEXT_SIZE);
                canvas.drawText(((LogiticsEntity.LogisticsBean) data.get(i)).getTime() + "", left * 2 + radius * 2 + 10, interval + heightList.get(i) + heightData + top, paint);
                //文字换行
                TextPaint textPaint = new TextPaint();
                textPaint.setColor(getResources().getColor(R.color.color_gray_d9d9d9));
                textPaint.setTextSize(CONTENT_TEXT_SIZE);
                textPaint.setAntiAlias(true);


                /**
                 * 分割带电话的数据  todo 暂时不需要此功能
                 * */
//                String[] splitData=splitString(((LogiticsEntity.LogisticsBean)data.get(i)).getContent()+"");
//                if(splitData!=null){
////                    说明有电话号码
        ...
        if (splitData.length <= 1) {
            ...
            if ((int) (windowWidth * 0.8) - layout.getLineWidth(layout.getLineCount() - 1) > phoneNumberWidth) {
                ...
                if ((int) (windowWidth * 0.8) - layout.getLineWidth(layout.getLineCount() - 1) - phoneNumberWidth > layoutLast.getLineWidth(0)) {
                    //4.1.如果一行就可以绘制完成
                    mPaintPhone.setColor(getResources().getColor(isTop ? R.color.colorPrimary : R.color.color_gray_d9d9d9));
                    ...
                    //获取字符能显示的最大Length
                    double maxLength = percentLast / CONTENT_TEXT_SIZE;//这里的35.0是一个中文字体的大小
