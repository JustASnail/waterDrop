...
public class WaterDropMixPushMessageHandler implements MixPushMessageHandler {

    @Override
    public boolean onNotificationClicked(Context context, Map<String, String> payload) {

        LogUtil.i(WaterDropMixPushMessageHandler.class.getSimpleName(), "rev miPushMessage payload " + payload);

        String sessionId = payload.get("targetId");
        String type = payload.get("msgtype");
        //
    ...
    private ComponentName initLaunchComponent(Context context) {
        ...
        StatusBarNotificationConfig config = AppCache.getNotificationConfig();
