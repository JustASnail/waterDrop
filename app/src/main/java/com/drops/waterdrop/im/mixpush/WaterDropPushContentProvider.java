package com.drops.waterdrop.im.mixpush;

import android.text.TextUtils;

import com.drops.waterdrop.app.AppCache;
import com.drops.waterdrop.model.PushCotentModel;
 ...
 * 示例：
 * 1.自定义的推送文案
 * 2.自定义推送 payload 实现特定的点击通知栏跳转行为{@link WaterDropMixPushMessageHandler}
 * <p>
 * 如果自定义文案和payload，请开发者在各端发送消息时保持一致。
 */

public class WaterDropPushContentProvider implements CustomPushContentProvider {
    ...
    private String getContent(IMMessage message) {
        ...
        NimUserInfo userInfo = NimUserInfoCache.getInstance().getUserInfo(AppCache.getAccount());
        if (userInfo == null) {
            NimUserInfoCache.getInstance().getUserInfoFromRemote(AppCache.getAccount(), null);
        }
    ...
    private Map<String, Object> getPayload(IMMessage message) {
        ...
        PushCotentModel model = new PushCotentModel();
        model.setMsgtype(message.getSessionType().getValue());
        model.setTargetId(message.getSessionId());


//        payload.put("msgtype", message.getSessionType());
        if (message.getSessionType() == SessionTypeEnum.Team) {
//            payload.put("targetId", message.getSessionId());
            model.setSchema("YSWaterDrop://NaviPush/NTESSessionViewController?teamId=" + message.getSessionId());

//            payload.put("schema", "YSWaterDrop://NaviPush/NTESSessionViewController?teamId=" + message.getSessionId());
        } else if (message.getSessionType() == SessionTypeEnum.P2P) {
//            payload.put("targetId", message.getSessionId());
            model.setSchema("YSWaterDrop://NaviPush/NTESSessionViewController?sessionId=" + AppCache.getAccount());

//            payload.put("schema", "YSWaterDrop://NaviPush/NTESSessionViewController?sessionId=" + message.getSessionId());
        }

        Gson gson = new Gson();
        String str = gson.toJson(model);
        payload.put("data", str);
