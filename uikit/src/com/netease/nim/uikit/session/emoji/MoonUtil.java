...
public class MoonUtil {
    ...
    private static class CustomUrlSpan extends ClickableSpan {
        ...
        public void onClick(View widget) {
            // 在这里可以做任何自己想要的处理
            SessionAutoLinkClickListener sessionAutoLinkClickListener = NimUIKit.getSessionAutoLinkClickListener();
            if (!TextUtils.isEmpty(url) && sessionAutoLinkClickListener != null) {
                sessionAutoLinkClickListener.onWebLinkClickListener(context, url);
            }

        }
    }

    public static CustomUrlSpan getCustomUrlSpan(Context context, String url) {
        return new CustomUrlSpan(context, url);
    }
}
