...
public class NimUserInfoCache {
    ...
    public String getAlias(String account) {
        MyFriendEntity friend = FriendDataCache.getInstance().getFriendByAccount(account);
        if (friend != null && !TextUtils.isEmpty(friend.getFNickName())) {
            return friend.getFNickName();
        }
        return null;
    }
    ...
    private void addOrUpdateUsers(final List<NimUserInfo> users, boolean notify) {
        ...
        FriendDataCache.getInstance().buildCache();//重新刷新好友资料
