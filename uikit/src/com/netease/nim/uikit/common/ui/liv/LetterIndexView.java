...
public class LetterIndexView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        float letterHeight = (((float)getHeight())- ScreenUtil.dip2px(80))/ letters.length;//修改内容高度
        float width = getWidth();
        float textSize = letterHeight * 5 / 7;//修改字体
        mPaint.setTextSize(textSize);
        for (int i = 0; i < letters.length; ++i) {
            float halfWidth = width / 2;
            float letterPosY = letterHeight * i + textSize + ScreenUtil.dip2px(40);
